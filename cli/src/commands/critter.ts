...
const spawn = {
    ...
    handler: async (ctx) => {
        ...
        await player.dispatchAndWait(
            { name: 'SPAWN_CRITTER', args: [unitId, randomSize || 10] },
            { name: 'MOVE_CRITTER', args: [unitId, q, r, s] }
        );
    },
};
...
const ticker = {
    ...
    handler: async (ctx) => {
        ...
        const res: any = await pipe(
            client.query(GetTilesDocument, { gameID: ctx.game }, { subscribe: false }),
            take(1),
            toPromise
        );
        ...
        while (true) {
            ...
            try {
                const res: any = await pipe(
                    client.query(GetWorldDocument, { gameID: ctx.game }, { subscribe: false }),
                    take(1),
                    toPromise
                );
                if (!res?.game?.state) {
            ...
            if (world.critters.filter((c) => !!c.nextLocation).length === 0) {
                ...
                sendHome = false;
                const actions = world.critters
                    .filter((c) => !!c.nextLocation)
                    .map((c) => {
                        const [q, r, s] = randomSpawnQRS();
                        return {
                            name: 'MOVE_CRITTER',
            ...
            for (let [_id, critter] of critters) {
                ...
                if (feelingRandom || critter.mode === 'random') {
                    ...
                    const validNeighbours = getNeighbourCoords(q, r, s)
                        .filter((nc) =>
                            validCoords.some((vc) => `${nc.q}:${nc.r}:${nc.s}` === `${vc.q}:${vc.r}:${vc.s}`)
                        )
                        .map(({ q, r, s }) => [q, r, s]);
                    ...
                    while (
                        backtrackAvoids < 5 &&
                        q === critter.prevCoords.q &&
                        r === critter.prevCoords.r &&
                        s === critter.prevCoords.s
                    ) {
                    ...
                    const targetTileId = CompoundKeyEncoder.encodeInt16(
                        NodeSelectors.Tile,
                        0,
                        critter.target.q,
                        critter.target.r,
            ...
            // buildings attack critters
            const buildingHasFired = new Map<string, boolean>();
            world.buildings
                .filter((b) => !!b.location?.tile)
                .forEach((building) => {
                    if (buildingHasFired.get(building.id)) {
                        return;
                    }
                    for (let [_id, critter] of critters) {
                        const critterTile = tiles.find((t) => t.id === critter.nextTile);
                        ...
                        const size = (bag?.slots || []).find((slot) => slot.key === 1)?.balance || 0;
                        const radius = size / 10.0;
                        const distanceToEdgeOfCritter = distance - radius;
                        const hp = Math.max(25 - distance * 8, 1);
                        if (distanceToEdgeOfCritter < 4) {
                            // towers can hit from 4 tiles away
                            ...
                            if (getBuildingCategory(building.kind) == BuildingCategory.TOWER) {
                                ...
                                buildingHasFired.set(building.id, true);
                            }
                        }
                    }
                });
...
export const critter = {
    ...
    builder: (yargs) => yargs.command(spawn).command(ticker),
};
