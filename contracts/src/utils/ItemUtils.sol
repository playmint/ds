...
library ItemUtils {
    ...
    function GreenGoo() internal pure returns (bytes24) {
        return Node.Item([uint32(1), uint32(0), uint32(0)], true);
    }

    function BlueGoo() internal pure returns (bytes24) {
        return Node.Item([uint32(0), uint32(1), uint32(0)], true);
    }

    function RedGoo() internal pure returns (bytes24) {
        return Node.Item([uint32(0), uint32(0), uint32(1)], true);
    }

    function MysteryItem() internal pure returns (bytes24) {
        return Node.Item([uint32(1), uint32(1), uint32(1)], true);
    }
    ...
    function register(Game ds, ItemConfig memory cfg) internal returns (bytes24) {
        ...
        bytes24 itemKind = Node.Item(outputItemAtoms, cfg.stackable);
