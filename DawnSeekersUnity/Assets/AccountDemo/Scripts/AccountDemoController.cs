...
public class AccountDemoController : MonoBehaviour
{
    ...
    private void SignIn_Clicked()
    {
        ...
        var authMessage = Sha3Keccack.Current.CalculateHash(
            Encoding.Unicode
                .GetBytes($"\x19Ethereum Signed Message:\n{authData.Length}")
                .Concat(authData)
                .ToArray()
        );
    ...
    private void GetAccounts_Clicked()
    {
        if (AccountManager.Instance.IsConnected())
        {
            ...
            _client.ExecuteQuery(
                GetAccountsGQL.GetAccountsDocument,
                variables,
                (Message response) =>
                {
                    // TODO error responses coming back as data!?
                    switch (response.Type)
                    {
                        case MessageType.GQL_DATA:
                            ...
                            var query =
                                response.Result.Data.ToObject<Cog.GraphQL.Generated.Types.Query>();
    ...
    private void GetAccount_Clicked()
    {
        ...
        if (AccountManager.Instance.IsConnected())
        {
            ...
            _client.ExecuteQuery(
                GetAccountGQL.GetAccountDocument,
                variables,
                (Message response) =>
                {
                    DisplayMessage(response.Result.Data.ToString());
    ...
    private void PrivateKeyConnectButton_Clicked()
    {
        ...
        {
            DisplayError(
                "Private Key Connect NOT Available, if running in web useMetamask or similar"
            );
        }
    }
    ...
    private void SignData_Clicked()
    {
        ...
        AccountManager.Instance.SignMessage(
            "Hello World",
            (signedMessage) => DisplayMessage(signedMessage),
    ...
    private void SignSession_Clicked()
    {
        ...
        AccountManager.Instance.SignSession(
            "Hello World",
            (signedMessage) => DisplayMessage(signedMessage),
            (error) => DisplayError(error)
        );
