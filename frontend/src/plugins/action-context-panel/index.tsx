...
const Construct: FunctionComponent<ConstructProps> = ({
    ...
    const { path, valid } = useMemo(() => {
        ...
        const path = getPath(tiles, tilesModifiedByPlugins, buildings, fromTile, toTile);
    ...
    }, [mobileUnit, tiles, selectedTiles, tilesModifiedByPlugins, buildings]);
    ...
    const { path, valid } = useMemo(() => {
        ...
        return {
            ...
            valid:
                path.length > 0 &&
                !isImposible &&
                !(targetTile && isBlockerTile(targetTile, tilesModifiedByPlugins)) &&
...
const Combat: FunctionComponent<CombatProps> = ({
    ...
    const { path, attackers, defenders, defenceTile, defenceTileBuilding, valid, reason } = useMemo(() => {
        ...
        const pathToEitherBuildingOrAttackTile = getPath(tiles, tilesModifiedByPlugins, buildings, fromTile, toTile);
    ...
    }, [mobileUnit, tiles, mobileUnits, selectedTiles, buildings, sessions, tilesModifiedByPlugins]);
...
export const ActionContextPanel: FunctionComponent<ActionContextPanelProps> = ({ tilesModifiedByPlugins }) => {
    ...
    if (intent === CONSTRUCT_INTENT) {
        return (
            <Construct
                ...
                player={player}
                tiles={tiles || []}
                tilesModifiedByPlugins={tilesModifiedByPlugins || []}
                bags={world?.bags || []}
